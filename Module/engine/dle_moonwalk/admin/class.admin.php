...
class AdminTemplate
{
	...
	public function __construct($db, $member_id, $user_group, $config, $module_config, $module_lang, $cat_info, $dle_login_hash)
    {
        $this->db = $db;
...
<!doctype html>
	<html lang="ru">
		<head>
			...
			<!-- Dashboard Core -->
			<link href="{$this->dle_config['http_home_url']}engine/dle_moonwalk/admin/assets/css/dashboard.css" rel="stylesheet" />
			<link href="{$this->dle_config['http_home_url']}engine/dle_moonwalk/admin/assets/css/chosen.min.css" rel="stylesheet" />
			<link href="{$this->dle_config['http_home_url']}engine/dle_moonwalk/admin/assets/css/toastr.min.css" rel="stylesheet" />
			<link href="{$this->dle_config['http_home_url']}engine/dle_moonwalk/admin/assets/css/jquery.sweet-modal.min.css" rel="stylesheet" />

			<script src="{$this->dle_config['http_home_url']}engine/dle_moonwalk/admin/assets/js/vendors/jquery-3.2.1.min.js"></script>
			<script src="{$this->dle_config['http_home_url']}engine/dle_moonwalk/admin/assets/js/vendors/bootstrap.bundle.min.js"></script>
			<script src="{$this->dle_config['http_home_url']}engine/dle_moonwalk/admin/assets/js/core.js"></script>
	</head>
	<body>
		<div class="page">
			<div class="page-main">
				<div class="header py-4">
					<div class="container">
						<div class="d-flex">
							<a class="header-brand" href="{$PHP_SELF}?mod=dle_moonwalk">
								{$this->module_lang['name']}
							</a>
							<div class="d-flex order-lg-2 ml-auto">
								<div class="nav-item d-none d-md-flex">
									<a href="https://moonwalk.lazydev.pro/" class="btn btn-sm btn-outline-success" target="_blank">{$this->module_lang['header_doc']}</a>&nbsp;&nbsp;
...
HTML;
	...
	public function showTrInline($name, $description, $type, $data)
	{
echo <<<HTML
<tr>
	<td>
...
return <<<HTML
	<textarea style="min-height:150px;max-height:150px;min-width:333px;max-width:100%;" class="form-control" name="{$data[0]}"{$input_elemet}>{$data[1]}</textarea>
HTML;
	}
	...
	public function select_sortForm($data)
	{
		...
		foreach ($data[1] as $key => $val) {
			if (strpos($key, 'xf_') !== false && !$opt) {
				$opt = true;
				$output = "<optgroup label=\"{$this->module_lang[147]}\">" . $output . "</optgroup><optgroup label=\"{$this->module_lang[148]}\">";
			}
			if ($data[2]) {
...
HTML;
	...
	{
		if ($ch = curl_init()) {
			curl_setopt($ch, CURLOPT_URL, 'https://api.github.com//repos/lazyphpdev/DLE-Moonwalk/releases/latest');
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_USERAGENT, 'lazyphpdev');
