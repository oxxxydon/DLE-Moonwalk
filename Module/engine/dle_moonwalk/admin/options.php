...
echoheader('<i class="fa fa-cogs position-left"></i> ' . $dle_moonwalk_admin_lang['name'], $dle_moonwalk_admin_lang['header_title'] . $dle_moonwalk_admin_lang['name']);
...
<script>
	...
	function ShowHide(d) {
		console.log($(d).text());
		if ($(d).text() === 'Описание полей') {
			$('#content_help').show();
			$(d).text('Скрыть описание полей');
		} else {
			$('#content_help').hide();
			$(d).text('Описание полей');
		}
	}
</script>
...
echo <<<HTML
	...
	<div id="block_2" class="panel panel-flat" style='display:none'>
		...
		<div class="alert alert-info alert-styled-left alert-arrow-left alert-component text-size-small" style="margin-bottom:0px!important;box-shadow:none!important;">
			Для того чтобы модуль начал заполнять информацию, Вам необходимо заполнить шаблон. В шаблон заполнения данных необходимо добавить переменную {X}, где X - значение поля. Также можно использовать связку [tag-X]...[/txg-X], которые выводят текст указанный в них.
			<br><br>
			<button style="border-radius: 0;background: #fff;border: 1px solid #009688;color: #000;width: 100%;" onclick="ShowHide(this); return false;" class="btn bg-teal btn-raised btn-sm">Описание полей</button>
...
showLazyRow($dle_moonwalk_admin_lang[63], $dle_moonwalk_admin_lang[64], lazySelect(['serial[season_format]', [1 => $dle_moonwalk_admin_lang[65], 2 => $dle_moonwalk_admin_lang[66], 3 => $dle_moonwalk_admin_lang[67]], true, $dle_moonwalk_config['serial']['season_format'], false, ($dle_moonwalk_config['serial']['season_xfield'] ? false : true)]));
showLazyRow($dle_moonwalk_admin_lang[68], $dle_moonwalk_admin_lang[69], lazySelect(['serial[seria_format]', [1 => $dle_moonwalk_admin_lang[70], 2 => $dle_moonwalk_admin_lang[71], 3 => $dle_moonwalk_admin_lang[72]], true, $dle_moonwalk_config['serial']['seria_format'], false, ($dle_moonwalk_config['serial']['seria_xfield'] ? false : true)]));
echo <<<HTML
<tr>
...
echo <<<HTML
	...
	<div id="block_5" class="panel panel-flat" style='display:none'>
		<div class="panel-body">{$dle_moonwalk_admin_lang[4]}</div>
		<div class="table-responsive">
			<table class="table table-striped">
HTML;
...
HTML;
require_once ENGINE_DIR . '/dle_moonwalk/admin/skin.php';
echofooter();
?>